...
endpoint_salto = Variable.get("salto_endpoint_inclusao_cpf")
...
def erro_coleta_token_graphql(ti):
    ...
    data = {
            "channel": "C03B1JPL46N",
            "text": f"[AIRFLOW] Ouve uma falha ao tentar coletar o token de acesso - DAG: Salto_Inclui_CPF - Task: Coleta_Token_Graphql - Erro: {erro}"
        }
...
def erro_coleta_cpf_salto(ti):
    ...
    data = {
            "channel": "C03B1JPL46N",
            "text": f"[AIRFLOW] Ouve uma falha ao tentar coletar os CPFs na API da Unimed Salto - DAG: Salto_Inclui_CPF - Task: Coleta_CPF_Salto - Erro: {erro}"
        }
    print("--------------- Chamando endpoint do Slack ---------------")
    r = requests.post(endpoint_slack, data=json.dumps(data),  headers={"Content-type" : "application/json",  "Authorization" : token_auth_slack})
...
def envia_dados_graphql(ti):
    ...
    try:
        ...
        r = requests.post("https://graphql.laura-br.com/api/v1/addRemoteCPF", data=json.dumps(data), headers={"Content-type" : "application/json", "Authorization" : graphQL_token})
...
def valida_envia_dados_graphql(ti):
    ...
    if  retorno_envio_dados == ["HTTP"] or retorno_envio_dados == ["conexão"] or retorno_envio_dados == ["timeout"] or retorno_envio_dados == ["exceção"]:
        print("--------------- Falha ao enviar os CPFs para o GraphQL ---------------")
        return 'Erro_Envia_Dados_Graphql'
    else:
        print("--------------- Sucesso ao enviar os CPFs ao GraphQL, encaminhando para o proximo passo ---------------")
        return 'Sucesso_Inclui_CPF_Salto' 
...
def erro_envia_dados_graphql(ti):
    ...
    data = {
            "channel": "C03B1JPL46N",
            "text": f"[AIRFLOW] Ouve uma falha ao tentar enviar os CPFs para a API do GraphQL - DAG: Salto_Inclui_CPF Task: Envia_Dados_GraphQL - Erro: {erro}"
        }
    print("--------------- Chamando endpoint do Slack ---------------")
    r = requests.post(endpoint_slack, data=json.dumps(data),  headers={"Content-type" : "application/json",  "Authorization" : token_auth_slack})

#Função que finaliza a DAG com sucesso
def sucesso_inclui_cpf_salto():
    print("--------------- Sucesso ao incluir os CPFs no GraphQL ---------------")  

with DAG('Salto_Inclui_CPF', start_date = datetime(2022,8,15), schedule_interval = '0 4 * * *', catchup = False, dagrun_timeout=timedelta(minutes=15)) as dag:
    ...
    sucesso_inclui_cpf_salto = PythonOperator(
        task_id = 'Sucesso_Inclui_CPF_Salto',
        python_callable = sucesso_inclui_cpf_salto
    )
    ...
    valida_cpf_salto >> envia_dados_graphql >> valida_envia_dados_graphql >> [erro_envia_dados_graphql, sucesso_inclui_cpf_salto]